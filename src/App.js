...
export default function App() {
  ...
  async function handleLikeRepository(id) {
    ...
    let newRepositorie = response.data;
    const newData = repositories.map(repositorie => repositorie.id === id ? newRepositorie : repositorie )
          ...
          renderItem={({ item: repositorie  }) => (
            <View style={styles.repositoryContainer}>
              ...
              <View style={styles.techsContainer}>
                {
                  repositorie.techs.map((item) =>{
                    return (
                      <Text key={item} style={styles.techList}>
                        {item}
                      </Text>
                    )
                  })
